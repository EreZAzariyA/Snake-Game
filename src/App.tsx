...
function App() {
  ...
  const checkCollision = (head: number[]) => {
    for (let i = 1; i < head.length; i++) {
      if (head[i] < 0 || head[i] * scale >= canvasY) return true;
    }
  ...
  const runGame = () => {
    ...
    const newNameHead = [newSnake[0][0] + direction[0], newSnake[0][1] + direction[1]];
    newSnake.unshift(newNameHead);
    if (checkCollision(newNameHead)) {
      setDelay(null);
      setGameOver(true);
      handleSetScore();
    }
